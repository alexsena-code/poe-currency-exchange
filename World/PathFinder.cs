...
/// <summary>
/// Dijkstra por CAMPO DE DISTÂNCIA a partir do alvo (portado do plugin Radar, comprovado):
/// RunFirstScan constrói o campo (caro, 1x por alvo, em background), FindPath segue o campo (barato).
/// Grid walkable = valores {1..5} do RawPathfindingData do ExileCore.
/// </summary>
public class PathFinder
{
    ...
    public bool IsPathable(Vector2i tile) => IsTilePathable(tile);

    private bool IsTilePathable(Vector2i tile)
    {
        if (tile.X < 0 || tile.X >= _dimension2) return false;
        if (tile.Y < 0 || tile.Y >= _dimension1) return false;
    ...
    private static IEnumerable<Vector2i> GetNeighbors(Vector2i tile) => NeighborOffsets.Select(offset => tile + offset);

    private static float GetExactDistance(Vector2i tile, Dictionary<Vector2i, float> dict) => dict.GetValueOrDefault(tile, float.PositiveInfinity);
    ...
    public IEnumerable<List<Vector2i>> RunFirstScan(Vector2i start, Vector2i target)
    {
        ...
        if (!ExactDistanceField.TryAdd(target, new Dictionary<Vector2i, float>())) yield break;
        ...
        {
            if (!IsTilePathable(coord)) return;
            if (localBacktrackDictionary.ContainsKey(coord)) return;
        ...
        while (queue.TryRemoveTop(out var top))
        {
            ...
            if (reversePath.Count == 0 && current.Equals(start))
            {
                ...
                { reversePath.Add(previous); it = previous; }
                yield return reversePath;
            }

            foreach (var neighbor in GetNeighbors(current)) TryEnqueueTile(neighbor, current, currentDistance);
            if (sw.ElapsedMilliseconds > 100) { yield return reversePath; sw.Restart(); }
        }
        ...
        if (_dimension1 * _dimension2 < exactDistanceField.Count * (sizeof(int) * 2 + Unsafe.SizeOf<Vector2i>() + Unsafe.SizeOf<float>()))
        {
            var directionGrid = _grid
                .AsParallel().AsOrdered().Select((r, y) => r.Select((_, x) =>
                {
                    ...
                    if (float.IsPositiveInfinity(GetExactDistance(coordVec, exactDistanceField))) return (byte)0;
                    ...
                    if (float.IsPositiveInfinity(clndistance)) return (byte)0;
    ...
    public bool HasField(Vector2i target) => DirectionField.ContainsKey(target) || ExactDistanceField.ContainsKey(target);

    public List<Vector2i> FindPath(Vector2i start, Vector2i target)
    {
        if (DirectionField.GetValueOrDefault(target) is { } directionField)
        {
            if (start.Y < 0 || start.Y >= directionField.Length || start.X < 0 || start.X >= directionField[0].Length) return null;
            if (directionField[start.Y][start.X] == 0) return null;
